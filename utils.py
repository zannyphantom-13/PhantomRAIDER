...
def help():
    ...
    venice [prompt]            --> Ask Venice AI a question directly
...
def get_shell(ip,port):
    ...
    while True:
        ...
        while True:
            ...
            while message_to_send.strip().startswith("venice "):
                prompt = message_to_send.strip()[7:]
                print(stdOutput("info") + "\033[0mThinking...")
                response = chat_with_venice(prompt)
                print("\033[1m\033[32m[Venice AI]:\033[0m\n" + response + "\n")
                message_to_send = input("\033[1m\033[36mInterpreter:/> \033[0m")+"\n"
...
def chat_with_venice(user_prompt):
    try:
        from openai import OpenAI
        client = OpenAI(
            api_key="VENICE_ADMIN_KEY_vyvcqcfZdSePOWRxPXGy-0PRTtmAFJmfnmV4Bfqxnd", 
        ...
        return "Error: openai library not installed. Run 'py -m pip install openai'"
    except Exception as e:
        return f"Error connecting to Venice AI: {e}"
